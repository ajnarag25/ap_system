...
if(isset($_POST['searchquery'])){
  $sql= "SELECT * FROM tbl_events WHERE title LIKE '%".$_POST['searchquery']."%' ";
}else{
...
<section class="page-content">
    ...
    <div class="search">
      ...
      <a href="event.php?reset=1">Reset</a>
    </form>
...
<div class="container-alumni">
  <div class="d-flex">
    ...
    <button type="button" class="btn btn-primary ml-auto mb-2" data-toggle="modal" data-target="#exampleModalLong">
      Add Event
    </button>
              ...
              <div class="form-group">
                ...
                <div class="form-group">
                  <label for="content">Text Content</label>
                    <textarea class="form-control" id="content" rows="3" placeholder="Enter Content" name="content" required></textarea>
                  </div>

                  <div class="form-group">
                <label for="banner">Banner</label>
                  <input type="text" class="form-control" id="banner" placeholder="Enter Banner" name="banner" required>
                </div>
                <div class="form-group">
...
{
    $sql2 = 'SELECT * FROM tbl_event_commits WHERE event_id='.$event['id'];
    $result2 = mysqli_query($conn, $sql2);
    $participating_count = mysqli_num_rows($result2);
    echo "
        <tr>
        <td>
       " . $event['id'] . "
       ...
       " . $event['schedule'] . "
        ...
        <td>
       " . mb_strimwidth($event['title'], 0, 50, "...") . "
        </td>

        <td>
       " . mb_strimwidth($event['content'], 0, 50, "...") . "
        </td>

        <td>
       " . mb_strimwidth($event['banner'], 0, 50, "...") . "
        </td>
  ...
  <!-- Button trigger modal -->
<button type='button' class='btn btn-link' data-toggle='modal' data-target='#edit".$event['id']."Modal'>
  Edit Event
</button>
  ...
  <!-- Modal -->
<div class='modal fade' id='edit".$event['id']."Modal' tabindex='-1' role='dialog' aria-labelledby='#edit".$event['id']."ModalLabel' aria-hidden='true'>
  <div class='modal-dialog' role='document'>
    <div class='modal-content'>
      <div class='modal-header'>
        <h5 class='modal-title' id='edit".$event['id']."ModalLabel'>Edit Event</h5>
        <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
          <span aria-hidden='true'>&times;</span>
        </button>
      </div>
        ...
        <input type='hidden' name='id' value='".$event['id']."'>
              <div class='form-group'>
                ...
                <div class='form-group'>
                  <label for='content'>Text Content</label>
                    <textarea class='form-control' id='content' rows='3' placeholder='Enter Content' name='content' required>".$event['content']."</textarea>
                  </div>

                  <div class='form-group'>
                <label for='banner'>Banner</label>
                  <input type='text' class='form-control' id='banner' placeholder='Enter Banner' name='banner' value='".$event['banner']."' required>
                </div>
                <div class='form-group'>
                  <label for='schedule'>Add Schedule:</label>
                    <input type='datetime-local' id='schedule' name='schedule' value='".date("Y-m-d\TH:i:s", strtotime($event['schedule']))."' required>
                      </div>
                       <div class='modal-footer'>
                         <button type='button' class='btn btn-secondary' data-dismiss='modal'>Close</button>
                            <button type='submit' class='btn btn-primary'>Save changes</button>
